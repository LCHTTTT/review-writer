...
def test_author_policy_preserves_technical_errors_and_reports_scientific_changes():
    ...
    before = {"chemical_identities": ["cubr"], "numbers": ["10"], "stereo": [], "required_labels": []}
    after = {**before, "chemical_identities": ["cubr2"], "numbers": ["20"]}
    ...
    assert errors == ["protected_callouts_changed", "protected_images_changed", "multiple_prose_blocks"]
    assert len(warnings) == len(changes) == 2
...
def test_question_routing_keeps_one_answer_and_explicit_related_reason(tmp_path, monkeypatch):
    ...
    with patch.object(module.loop, "call_json_model", return_value={"mode": "revision", "targets": ["foreign"]}):
        with pytest.raises(ValueError, match="Invalid conversation targets"):
