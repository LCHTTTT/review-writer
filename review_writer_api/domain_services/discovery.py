...
from review_writer_core.stages.discovery.records import (
    _candidate_id,
    _selected,
    discovery_coverage_diagnostics,
    discovery_search_record,
    publication_year as _publication_year,
)
...
class DiscoveryService(OwnedProjectService):
    ...
    def search_payload(self, principal, project_id, request):
    ...
    def validate_search_inputs(self, principal, project_id, payload):
        self._owned_project(principal, project_id)
        state = self.repository.get_stage_state(principal.user_id, project_id, "discovery")
        revision = state.revision if state else 0
        ...
        if type(expected) is not int or expected != revision:
            raise WorkflowConflict(
                "Discovery changed while the search was waiting or running. Start a new search.",
                details={"expected_revision": expected, "actual_revision": revision},
            )
    ...
    def replace_from_job(
        ...
        self.validate_search_inputs(principal, project_id, payload)
    ...
    def confirm(self, principal: Principal, project_id: str, revision: int) -> dict[str, Any]:
        ...
        if (
            current.get("coverage_confirmation_required")
            and current.get("coverage_decision") != "keep_local"
        ):
            raise WorkflowConflict(
                "Specialized retrieval normalization was unavailable. Confirm the general-search coverage before creating a new Matrix."
            )
