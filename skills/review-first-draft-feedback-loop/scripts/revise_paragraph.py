...
def revise(project, request):
    if request.get("route_only"):
        ...
        result = loop.call_json_model(
            "Route this chapter discussion. Return JSON {mode: 'question', targets: [paragraph_id], "
            ...
            "and drawn from allowed. If ambiguous choose one primary target to ask clarification, never fan out "
        ...
        if not isinstance(result, dict) or result.get("mode") not in {"question", "revision"}:
            raise ValueError("Could not determine the conversation scope. Please specify the target paragraph.")
        targets = result.get("targets")
        if not isinstance(targets, list) or not targets or any(not isinstance(p, str) or p not in allowed for p in targets):
            raise ValueError("Invalid conversation targets. Please specify the target paragraph.")
    ...
    if candidate:
        ...
        if human_revision:
            errors, warnings, changes = author_revision_findings(errors, warnings,
                loop.protected_signature(paragraph["text"]), loop.protected_signature(candidate))
