...
export function SectionContentNotice({ section }: { section?: ContentDiagnostics }) {
  ...
  const needsPolish = section.narrative_diagnostics?.review_status === "needs_revision" && !limited && !omitted.length;
  const tableFallback = (section.paper_presentation || []).filter((paper) => paper.table_fallback);
  if (!binding && !other && !shallow && !limited && !needsPolish && !tableFallback.length) return null;
  return <div className="message message-warning" role="status">
    <strong>{needsPolish ? text("待优化草稿：论证组织或表达仍可改善。", "Draft ready for refinement: organization or expression can improve.") : text(limited || omitted.length ? "章节已生成，部分内容存在限制。" : "章节已生成。", limited || omitted.length ? "Section generated with content limitations." : "Section generated.")}</strong>
    ...
    {section.narrative_diagnostics?.issues?.length ? <ul>{section.narrative_diagnostics.issues.map((issue, index) => <li key={index}>{issue}</li>)}</ul> : null}
