...
export function BibliographyResolutionPanel({
  ...
  return (
    <section className="bibliography-resolution-panel">
      ...
      {(audit.error || startAudit.error || resolve.error) ? <p className="message message-error"><LocalizedError error={(audit.error || startAudit.error || resolve.error)} /></p> : null}
    </section>
  );
}
